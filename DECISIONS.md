...
- Key decision: Build two categorical variables (Funding_Type and USnewsTop100Rank) alongside the continuous variables so we could compare admissions/test-score patterns across public vs. private schools and ranked vs. unranked schools
...
- Most surprising pattern: Total_Undergraduates is heavily right-skewed (skewness 3.02, kurtosis 11.5) — a handful of very large universities pull the mean (5,597) far above the median (2,429). Admissions_Rate is left-skewed the opposite way, with most schools clustered at high acceptance rates (70-93%) rather than being highly selective — over 90% of schools in the dataset aren't ranked in the US News Top 100.

## Assignment 4: Probability (2026-07-26)
- Normal vs. empirical, and why: Only SAT_Average, median ACT_Composite_50, the $30-48K income bracket, and (weakly) public out-of-state tuition were close to a normal distribution. Total_Undergraduates, tuition variables, Admissions_Rate, and the other income brackets were too skewed for normal-based probability, so empirical/percentile-based probabilities were used for those instead.

## Assignment 5: Inference (2026-08-09)
- What we tested, alpha, conclusion: Ran two one-sample t-tests at α = 0.05.
  1. H0: mean Admissions_Rate ≥ 50% vs. Ha: < 50% → failed to reject (t = 25.84, p = 1.0). Most universities in the dataset are not highly selective.
